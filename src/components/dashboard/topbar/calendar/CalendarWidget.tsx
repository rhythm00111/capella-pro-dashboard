    ...
    <div className="min-w-[320px]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-zinc-400">{currentTime}</p>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={handleToday}
            className="px-2.5 py-1 text-xs text-zinc-400 hover:text-white hover:bg-zinc-800/50 rounded-md transition-colors"
          >
            Today
          </button>
          <button
      ...
      {/* Month Name */}
      <p className="text-xs text-zinc-500 text-center mb-3">{monthName}</p>
      ...
      <div className="grid grid-cols-7 gap-1">
        {calendarData.map((day, index) => (
          <button
            ...
            className={cn(
              "relative h-9 w-9 flex flex-col items-center justify-center rounded-lg transition-all duration-150 text-xs",
      ...
      {selectedDate && (
        <div className="mt-4 pt-3 border-t border-zinc-800/30">
          <h3 className="text-xs font-medium text-zinc-400 mb-2">{formatSelectedDate()}</h3>
          {selectedEvents ? (
            <div className="space-y-1.5">
              {selectedEvents.map((event, index) => (
                <div
                  key={index}
                  className="bg-zinc-800/30 px-3 py-2 rounded-lg border-l-2 border-red-500"
                >
